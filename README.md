<div align="center">

# Jonny Terrero

**Biomedical Engineering student building health platforms and AI systems end to end.**  
From sensors and CAD to production web apps and multi-agent tooling.  
If it ships, it ships clean.

[Portfolio](https://jonnyterrero.github.io) · [LinkedIn](https://linkedin.com/in/jonnyterrero) · [GitHub](https://github.com/jonnyterrero) · [LeetCode](https://leetcode.com/jterrero16)

</div>

---

## Pinned projects

| Project | What it does |
|---------|--------------|
| [**HealthHelper**](https://github.com/jonnyterrero/HealthHelper) | Unified health hub — connects tracking apps into one dashboard (Next.js · Supabase · TS) |
| [**MindMap**](https://github.com/jonnyterrero/MindMap) | Mental health tracking — mood, anxiety, ADHD, migraines, longitudinal analytics |
| [**GastroGuard**](https://github.com/jonnyterrero/gastro-guard) | GI tracker with sensor integration and an ML-ready feature pipeline |
| [**HeartWire-OS**](https://github.com/jonnyterrero/HeartWire-OS) | Personal OS for coursework, projects, research, and agents |
| [**Workflows & Automations**](https://github.com/jonnyterrero/workflows-and-automations) | Claude/MCP setup, Second Brain automations, and nested AI research helper |
| [**SkinTrack+**](https://github.com/jonnyterrero/SkinTrack-) | Skin condition progression tracking via image capture and analysis |

---

## Stack

```
Languages     Python · TypeScript · C/C++ · MATLAB · SQL
Frontend      Next.js · React · React Native
Backend       Supabase · Firebase · Prisma · Vercel
AI & Data     OpenAI Agents SDK · Claude · NumPy · SciPy · Matplotlib
Hardware      Arduino · Servos · Sensors · SolidWorks
Infra         Docker · GitHub Actions · Make.com · Proxmox
```

---

## Recent activity

<!--RECENT_START-->
Auto-updated from latest public activity.
- **jonnyterrero** — updated 2026-09-06 10:21:08 UTC  
  [repo](https://github.com/jonnyterrero/jonnyterrero) • ★0 • forks:0
- **Neetcode-Problems** — updated 2026-09-06 03:25:51 UTC  
  [repo](https://github.com/jonnyterrero/Neetcode-Problems) • ★0 • forks:0
- **MindMap** — updated 2026-09-04 19:05:02 UTC  
  [repo](https://github.com/jonnyterrero/MindMap) • ★1 • forks:0
- **workflows-and-automations** — updated 2026-09-04 16:28:12 UTC  
  [repo](https://github.com/jonnyterrero/workflows-and-automations) • ★1 • forks:0
- **JonnyTerrero.github.io** — updated 2026-08-30 04:42:30 UTC  
  [repo](https://github.com/jonnyterrero/JonnyTerrero.github.io) • ★0 • forks:0
- **HeartWire-OS** — updated 2026-08-30 03:58:07 UTC  
  [repo](https://github.com/jonnyterrero/HeartWire-OS) • ★1 • forks:0
- **SkinTrack-** — updated 2026-07-12 16:12:36 UTC  
  [repo](https://github.com/jonnyterrero/SkinTrack-) • ★1 • forks:0
<!--RECENT_END-->

### Latest commits

<!--COMMITS_START-->
- **jonnyterrero**: chore: auto-update README [skip ci]  
  [b92d1e3](https://github.com/jonnyterrero/jonnyterrero/commit/b92d1e331cdafa41096a874ea56a805ff7140576) • 2026-09-06 10:21:07 UTC
- **Neetcode-Problems**: Add: eating-bananas - submission-0  
  [9b39e6b](https://github.com/jonnyterrero/Neetcode-Problems/commit/9b39e6b1ddf6b16251b43347faae126be320d888) • 2026-09-06 03:25:50 UTC
- **MindMap**: docs(ml): add real training and hosting handoff  
  [e44ebab](https://github.com/jonnyterrero/MindMap/commit/e44ebab7ab4f5cc06017d5d6bd3c4c5c8a1dfa52) • 2026-09-04 19:04:58 UTC
- **workflows-and-automations**: Merge pull request #16 from jonnyterrero/claude/public-account-connection-n696ny  
  [c41f1af](https://github.com/jonnyterrero/workflows-and-automations/commit/c41f1af859473dfa38113ddf2fcef7a9fd8231d7) • 2026-08-27 23:07:23 UTC
<!--COMMITS_END-->

---

## Contact

- **Portfolio:** [jonnyterrero.github.io](https://jonnyterrero.github.io)
- **LinkedIn:** [linkedin.com/in/jonnyterrero](https://linkedin.com/in/jonnyterrero)
- **Email:** dijinvestments3@gmail.com

<div align="center">

*Ships clean. Breaks nothing. Learns everything.*

</div>
